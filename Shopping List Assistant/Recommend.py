...
def recommend_product(product_count, cluster_number, current_basket):
    ...
    Parameters:
    -------------------
    product_count: intThe number of products to be recommended
    cluster_number: int The cluster number the user belongs to
    current_basket: list A list of basket items

    Returns:
    -------------------
    recommendations: listA list of product recommendations based on the input
    """
    ...
    with open('product_lookup.pkl', 'rb') as file:product_lookup = pickle.load(file)
...
def retrieve_recommendations(last_purchase_history):
    ...
    recommendations = recommend_product( 10, cluster_number, history)  # Set product_count to 10
